"""End-to-end verification for data/2026-04-25/ after discover_v3.

Checks:
  (1) per-category row counts + bio-crawl-derived breakdown
  (2) KR exclusion (must be 0 in every cat)
  (3) cross-date dedup against 2026-04-24 (must be 0)
  (4) per-row mapping coverage (unique niche / angle == row count is the
      hand-crafted bar; we also report it as a sanity gauge)
  (5) 1 record = 1 line — raw line count must equal record + 1
  (6) sample-5 random bio-crawl rows: confirm email exists in source HTML
"""
...
OLD = ROOT / "data" / "2026-04-24"
NEW = ROOT / "data" / "2026-04-25"
CATS = ["career", "fitness", "nutrition", "lifestyle", "beatmakers"]
...
def main() -> None:
    ...
    # Collect old keys (cross-date dedup target)
    old_e, old_u = set(), set()
    for cat in CATS:
        for r in read(OLD / f"{cat}_enriched.csv"):
    ...
    fmt = "{:11s} | rows={:>3d} | KR={:>1d} | xdate-dup={:>2d} | uniq-niche={:>3d} | uniq-angle={:>3d} | bio-derived={:>2d} | website={:>3d} | tech={:>3d}"
    grand_rows = 0
    grand_dup = 0
    for cat in CATS:
        rows = read(NEW / f"{cat}_enriched.csv")
        grand_rows += len(rows)
        kr = sum(1 for r in rows if r.get("country", "").upper() == "KR")
        ...
        print(fmt.format(cat, len(rows), kr, dup, un_n, un_a, bio, web, tech))

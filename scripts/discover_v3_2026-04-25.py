"""A+B+C combined discovery for data/2026-04-25/.

A. MIN_SUBS = 1000   (smaller channels expose emails more often)
B. search().list with pageToken — 2 pages per keyword (~100 results each)
C. For channels with NO email in description: extract personal-domain +
   link-in-bio URLs from the description and crawl them with the project's
   Playwright WebsiteCrawler. The first non-role-based email found becomes
   the channel's email, with `source_url` pointing at the page it was scraped
   from (NOT the YouTube profile_url).

Stops cleanly on quotaExceeded. Appends to existing CSVs; never overwrites.
Excludes country=KR. Dedup against 2026-04-24 + current 2026-04-25.
Run order: beatmakers → career → nutrition → fitness → lifestyle (least
populated first, so partial quota still fills the smallest buckets).
"""
...
C_CRAWL_MIN_SUBS = 1500  # only crawl bio for channels >= this; saves time
PAGES_PER_KEYWORD = 2

# Categories ordered by current row count ascending (smallest first → safer
# under quota pressure).
RUN_ORDER = ["beatmakers", "career", "nutrition", "fitness", "lifestyle"]

KEYWORDS = {
    "career": [
        ("career coach", None),
        ("interview coach", None),
        ("resume tips", None),
        ("salary negotiation", None),
        ("LinkedIn growth coach", None),
        ("career advice hindi", "IN"),
        ("data science career", None),
        ("job hunt mentor", None),
    ],
    "fitness": [
        ("home workout", None),
        ("calisthenics tutorial", None),
        ("strength coach", None),
        ("running coach", None),
        ("yoga guide hindi", "IN"),
        ("entrenador personal", None),
        ("treino em casa", "BR"),
        ("kettlebell workout", None),
    ],
    "nutrition": [
        ("nutrition coach", None),
        ("registered dietitian", None),
        ("intermittent fasting coach", None),
        ("keto coach", None),
        ("plant based dietitian", None),
        ("diet plan hindi", "IN"),
        ("nutricionista", "BR"),
        ("diabetic diet", None),
    ],
    "lifestyle": [
        ("slow living vlog", None),
        ("minimalist lifestyle", None),
        ("daily vlog routine", None),
        ("expat life vlog", None),
        ("japan slow living vlog", "JP"),
        ("vlog brasileira diario", "BR"),
        ("intentional living", None),
        ("homestead vlog", None),
    ],
    "beatmakers": [
        ("beatmaker tutorial", None),
        ("fl studio tutorial", None),
        ("how to make trap beats", None),
        ("amapiano producer tutorial", None),
        ("afrobeat producer tutorial", None),
        ("drill beat tutorial", None),
        ("type beat producer", None),
        ("beatmaker español", None),
    ],
}
...
def channel_to_partial(it: dict) -> dict | None:
    ...
    email yet. Returns None if the channel fails hard filters (subs, KR)."""
    ...
    for url in candidates:
        ...
        for em in result.emails:
            if not is_role_based(em.email):
                ...
                rec["source_url"] = url
                rec["is_role_based"] = "0"
                # If a personal domain, fill website too
                rec["website"] = url
                return True
        # All role-based; take the first as last resort
        em = result.emails[0]
        rec["email"] = em.email
        rec["raw_context"] = flatten(em.raw_context)
        rec["source_url"] = url
...
def main() -> None:
    ...
    old_paths = (
        [DATED_OLD / f"{c}_enriched.csv" for c in RUN_ORDER]
        + [DATED_NEW / f"{c}_enriched.csv" for c in RUN_ORDER]
    )
    ...
    try:
        with WebsiteCrawler(
            ...
            for category in RUN_ORDER:
                ...
                # Phase 1: description-email pass
                phase1: list[dict] = []
                ...
                # Phase 2: bio-link crawl for those without desc emails
                phase2: list[dict] = []
    ...
    print("\nNew rows per category (these need _v3 mapping entries):")

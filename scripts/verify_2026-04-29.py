"""End-to-end verification for data/2026-04-29/ after discover_2026-04-29.

Checks:
  (1) per-category row counts + bio-crawl-derived breakdown + country mix
  (2) JP/KR exclusion (must be 0)
  (3) cross-date dedup against 2026-04-24 + 25 + 27 + 28 (must be 0)
...
NEW = ROOT / "data" / "2026-04-29"
CATS = ["meditation", "mindset", "fitness", "nutrition"]
# All categories that may appear in older sweep outputs
OLD_CATS = ["career", "fitness", "nutrition", "lifestyle", "beatmakers", "meditation"]
...
def main() -> None:
    ...
    print("\n=== (1b) country distribution (US ≥ 35% target) ===")
    for cat in CATS:
        rows = read(NEW / f"{cat}_enriched.csv")
        ...
        for c, n in ctry.most_common():
            pct = 100 * n / total
            print(f"    {c:8s}: {n:3d} ({pct:5.1f}%)")
        us_pct = 100 * ctry.get("US", 0) / total
        print(f"    → US share: {us_pct:.1f}% {'✓' if us_pct >= 35 else '✗'}")

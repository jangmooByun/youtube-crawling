"""PT / Strength / Hypertrophy / Powerlifting / Bodybuilding / CrossFit
sweep into `data/2026-04-28/`.

Why: 04-27 sweep (yoga / pilates / bodyweight) skewed IN-heavy because yoga
keywords surface Rishikesh TT schools + desi-style home-workout clones.
This sweep targets the gym/PT/strength vertical that is US/UK-dominated
(Mike Israetel/RP, Jeff Nippard, NASM/ACE/CSCS cert holders, CrossFit) —
keyword-as-natural-filter rather than regionCode bias.

Same engine as discover_2026-04-27_more.py (PAGES=4, MIN_SUBS=1000,
TARGET=300, EXCLUDE_COUNTRIES={KR,JP}). Cross-date dedup against
2026-04-24 + 2026-04-25 + 2026-04-27 fitness enriched CSVs.
"""
...
C_CRAWL_MIN_SUBS = 1000  # only crawl bio for channels >= this; saves time
PAGES_PER_KEYWORD = 4
...
# English-intent keywords targeting US/UK-dominant gym/PT/strength vertical.
# No region/language filter; keyword-as-natural-filter is the bias mechanism.
KEYWORDS = {
    "fitness": [
        # Online PT (general) — US/UK strong
        ("online personal trainer", None),
        ("online fitness coach", None),
        ("online strength coach", None),
        ("online lifting coach", None),
        # US-specific certifications — IN essentially absent
        ("NASM certified trainer", None),
        ("ACE certified personal trainer", None),
        ("CSCS certified coach", None),
        # Hypertrophy / Bodybuilding — RP / Jeff Nippard school
        ("hypertrophy coach", None),
        ("natural bodybuilding coach", None),
        ("online physique coach", None),
        ("muscle building coach online", None),
        # Powerlifting / Strength sports
        ("powerlifting coach online", None),
        ("barbell training program", None),
        ("online lifting program", None),
        # CrossFit — US-dominant
        ("online crossfit coach", None),
        ("crossfit program design", None),
        ("crossfit at home coach", None),
    ],
}

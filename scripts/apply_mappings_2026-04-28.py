"""Apply per-row mapping files to data/2026-04-28/*.csv."""
...
def main() -> None:
    ...
    for cat in CATEGORIES:
        base_path = SCRIPTS / f"{cat}_mapping_2026-04-28.py"
        if not base_path.exists():
            print(f"SKIP {cat}: no base mapping at {base_path.name}")
            continue
        mapping = load(base_path)
        for extra in ("_more", "_v3"):
            extra_path = SCRIPTS / f"{cat}_mapping_2026-04-28{extra}.py"

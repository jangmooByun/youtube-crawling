"""Apply per-row mapping files to data/2026-04-29/*.csv.

Looks for `<category>_mapping_2026-04-29.py` plus optional `_more` / `_v3`
suffix variants and merges them in order (later wins). Updates niche /
segment_original / industry / angle_to_take in-place.
"""
...
def main() -> None:
    ...
    for cat in CATEGORIES:
        base_path = SCRIPTS / f"{cat}_mapping_2026-04-29.py"
        if not base_path.exists():
            print(f"SKIP {cat}: no base mapping at {base_path.name}")
            continue
        mapping = load(base_path)
        for extra in ("_more", "_v3"):
            extra_path = SCRIPTS / f"{cat}_mapping_2026-04-29{extra}.py"
